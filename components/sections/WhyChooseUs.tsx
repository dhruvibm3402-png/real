...
export default function WhyChooseUs() {
  ...
  return (
    <section id="why-us" className="wu-section" style={{ background: '#fff', padding: '120px 0', fontFamily: 'Inter, Arial, sans-serif', overflow: 'hidden' }}>
      <div className="wu-wrap" style={{ maxWidth: 1440, margin: '0 auto', padding: '0 48px', marginBottom: 60 }}>
        <h2 className="wu-title" style={{ fontSize: 48, fontWeight: 400, lineHeight: 1.1, letterSpacing: '-0.02em', color: '#0f172a', textAlign: 'center', marginBottom: 60 }}>Why Aurora Estates</h2>
      </div>
      <div style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', overflow: 'hidden', position: 'relative', padding: '20px 0', WebkitMaskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)', maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}>
        <div style={{ display: 'flex', gap: 30, width: 'max-content', animation: 'marquee 25s linear infinite' }}>
          {[...items, ...items].map((item, i) => (
            <div key={i} className="wu-card" style={{ border: '1px solid rgba(14,165,233,0.15)', padding: '40px 60px', textAlign: 'center', borderRadius: 4, background: 'rgba(255,255,255,0.02)', minWidth: 350, boxShadow: '0 4px 20px rgba(0,0,0,0.05)', transition: 'transform 0.3s, border-color 0.3s' }}
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .wu-section { padding: 60px 0 !important; }
          .wu-title { font-size: 32px !important; margin-bottom: 30px !important; }
          .wu-wrap { padding: 0 20px !important; margin-bottom: 30px !important; }
          .wu-card { min-width: 250px !important; padding: 20px 30px !important; }
          .wu-card h3 { font-size: 16px !important; }
        }
      `}</style>
    </section>
  );
